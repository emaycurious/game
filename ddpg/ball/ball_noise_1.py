...
class Noise:
    def __init__(self, actor):
        self.theta, self.dt, self.sigma = 0.1, 0.01, 0.1        # sigma 影响比较小
        self.actor = actor
...
def play():
    ...
    # mind
    state_dim, action_dim, h1_node, h2_node, h3_node, q_node = 2, 2, 50, 25, 12, 1
    ddpg = DDPG(state_dim, action_dim, h1_node, h2_node, h3_node, q_node)
    ...
    while 1:
        ...
        while count < 200 and not done:  # 每一轮动 200 下
            ...
            a = ddpg.actor(torch.from_numpy(s + ddpg.noise()[0]).float()).detach().numpy()  # 最后存下 actor ok
            ...
            # 存进去
            ddpg.memory.put((s, a, r, s_next, done))  # for train
            count += 1
...
def save(ddpg):
    # 保存
    torch.save(ddpg.state_dict(), 'ddpg_model.pth')
    print('saved')
...
def test():
    ...
    state_dim, action_dim, h1_node, h2_node, h3_node, q_node = 2, 2, 50, 25, 12, 1
    ddpg = DDPG(state_dim, action_dim, h1_node, h2_node, h3_node, q_node)
    ddpg.load_state_dict(torch.load('ddpg_model.pth'))
