...
def get_target_reward(old_ball_xy, ball, target):
    reward = 0
    if abs(ball.x - target.x) < abs(old_ball_xy[0] - target.x) or abs(ball.y - target.y) < abs(old_ball_xy[1] - target.y):
        reward += 0.01
    else:
        reward -= 2
...
def play():
    ...
    # target
    target = Ball(bg_width - 20, 20, 10, 0, 1, Red)

    # mind

    # mind_1
    step_1 = 0.01
    mind_1 = Mind(2, 10, 2, step_1)
    ...
    while 1:
        ...
        ball.step = mind_1.outputs[0][0] * 100
