...
def get_target_reward(old_ball_xy, ball, target):
    reward = 0
    if abs(ball.x - target.x) < abs(old_ball_xy[0] - target.x) or abs(ball.y - target.y) < abs(
            old_ball_xy[1] - target.y):
        reward += 2
    else:
        reward -= 2
...
def play():
    ...
    # target
    target = Ball(200, bg_height/2, 10, 0, 1, Red)

    # mind

    # mind_1
    step_1 = 0.01
    mind_1 = Mind(2, 5, 2, step_1)
    ...
    # mind_2
    step_2 = 0.01
    mind_2 = Mind(mind_1.input_node + mind_1.output_node, 5, mind_1.output_node, step_2)
    ...
    while 1:
        ...
        mind_2.inputs = numpy.array([numpy.append(mind_1.inputs.T, mind_1.outputs)])
        mind_2.hiddens = numpy.tanh(mind_2.w_ih.dot(mind_2.inputs.T))
        mind_2.outputs = numpy.tanh(mind_2.w_ho.dot(mind_2.hiddens))

        print(mind_1.outputs)

        # 动一下（ 会有一个新的 ball.x, ball.y ）
        old_ball_xy = [ball.x, ball.y]
        ball.step = mind_1.outputs[0][0] + 2
        ball.jiao = mind_1.outputs[1][0] * 180 + 180  # (-1,1) * 180 => (-180,180) + 180 => (0,360)
