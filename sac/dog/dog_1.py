...
class Dog:
    def __init__(self, x, y, z, dq, kp, kd):
        # 位置
        self.x = x
        self.y = y
        self.z = z
    ...
    def get_reward(self, old_xyz, target_xyz):

        reward = 0

        # 靠近目标
        if (self.x - target_xyz[0]) ** 2 + (self.y - target_xyz[1]) ** 2 + (self.z - target_xyz[2]) ** 2 < \
                (old_xyz[0] - target_xyz[0]) ** 2 + (old_xyz[1] - target_xyz[1]) ** 2 + \
                (old_xyz[2] - target_xyz[2]) ** 2:
            print('close')
            reward += 0.01  # 和目标加分，相差大一些
        else:
            print('far')
            reward -= 0.5
...
def play():
    ...
    state_dim = 3  # (x,y,z)
    action_dim = 12 * 3  # 12 个关节 ( 3 个 dp,kp,kd)
    ...
    for n_epi in range(10000):
        ...
        while count < 1000 and not done:  # 每一轮，动 1000 步
            ...
            # 动一下
            s = np.array([dog.x, dog.y, dog.z])
            a, log_prob = sac.actor(torch.from_numpy(s).float())
            a = a.detach().numpy() * 2.0

            # move
            old_xyz = [dog.x, dog.y, dog.z]

            # 动 12 个关节 (每个关节对应 3 个节点，总共 36 个输出节点)
            for i in range(12):
                lowCmd.motorCmd[i].mode = 10
                lowCmd.motorCmd[i].dq = a[i * 3 + 0].item()
                lowCmd.motorCmd[i].Kp = a[i * 3 + 1].item()
                lowCmd.motorCmd[i].Kd = a[i * 3 + 2].item()

            for i in range(12):
                joint_pub[i].publish(lowCmd.motorCmd[i])

            s_next = np.array([dog.x, dog.y, dog.z])

            # reward
            r = dog.get_reward(old_xyz, target_xyz)  # 靠近目标

            # 碰到墙 (imu 传感器)

            sac.memory.put((s, a, r, s_next, done))  # 放进 memory
            count += 1
